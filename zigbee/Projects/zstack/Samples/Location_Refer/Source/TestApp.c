...
#define RECV_TIMEOUT         160  //设置一个序列号的超时时间，如果大于此还未发射表示未接满，直接发送
#define LAST_MOBILE_ID         4  //最后一个移动节点的ID，也可以理解为一共有的移动节点的个数
#define DELAY_TIME            18  //实际延迟是2*DELAY_TIME，指每次同发射端的延迟时间
...
int32 i_lastTimeDifference[LAST_MOBILE_ID];                            //上一次接收时间差的数组，用来判断是否本次测量是否正确的
...
void initP0()
{
   ...
   for(temp = 0;temp < LAST_MOBILE_ID;temp++)
   {
     i_lastTimeDifference[temp] = 0;               //将上次接收时间差数组清零
   }
...
uint16 Refer_ProcessEvent( byte task_id, uint16 events )
{
  ...
  if ( events & REFER_DELAYSEND_EVT )    
  {
    ...
    //将发送数组清零
    int i;
    for(i=0;i<LOCATION_REFER_POSITION_RSP_LENGTH;i++)
    {
      timeDiffMSG[i] = 0;     
    }
...
static void checkTime()
{
  ...
  else
  {
    ...
    tempTimeDifference = i_timeDifference[recvMobiID-1];
    ...
    //去除同上次测量的差值大于30cm的数据,不管本次结果是否正确，都记录下来
    else
    {
      if ( abs(i_timeDifference[recvMobiID-1] - i_lastTimeDifference[recvMobiID-1]) > 28236 )
      {
        i_timeDifference[recvMobiID-1] = 0xFFFFFF;
      }
      i_lastTimeDifference[recvMobiID-1] = tempTimeDifference;
    }
...
{  
  ...
  else
  {
    ...
    delaySend = 5 * (LOCATION_REFER_ID-1);
